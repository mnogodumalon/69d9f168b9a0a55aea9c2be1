...
export function BenutzerrollenDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: BenutzerrollenDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'benutzerrollen');
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "firstname": string | null, // Vorname\n  "lastname": string | null, // Nachname\n  "email": string | null, // E-Mail-Adresse\n  "phone": string | null, // Telefonnummer\n  "role": LookupValue | null, // Rolle (select one key: "contributor" | "facilitator" | "curator" | "ai_support" | "admin" | "viewer") mapping: contributor=Contributor, facilitator=Facilitator, curator=Curator, ai_support=AI-Support, admin=Admin, viewer=Viewer\n  "department": string | null, // Abteilung / Organisationseinheit\n  "active": boolean | null, // Aktiv\n}`;
  ...
  const DIALOG_INTENT = defaultValues ? 'Benutzerrollen bearbeiten' : 'Benutzerrollen hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              value={lookupKey(fields.role) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, role: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="role"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            <Label htmlFor="department">Abteilung / Organisationseinheit</Label>
            <Input
              id="department"
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 pt-1">
              <Checkbox
                ...
                onCheckedChange={(v) => setFields(f => ({ ...f, active: !!v }))}
              />
              <Label htmlFor="active" className="font-normal">Aktiv</Label>
            </div>
          </div>
          <DialogFooter>
